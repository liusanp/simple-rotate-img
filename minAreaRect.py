...
def preprocess(gray):
    ...
    # 3.膨胀和腐蚀操作的核函数
    dilation1_element = cv2.getStructuringElement(cv2.MORPH_RECT, (5, 10))
    ...
    # 7.存储中间图片
    cv2.imwrite('tmp/1sobel.png', sobel)
    cv2.imwrite('tmp/2binary.png', binary)
    cv2.imwrite('tmp/3dilation.png', dilation)
    cv2.imwrite('tmp/4erosion.png', erosion)
    cv2.imwrite('tmp/5dilation2.png', dilation2)
...
if __name__ == '__main__':
    x = 'C:\\Users\\liusa\\Desktop\\202010927zjbq2.jpg'
    img = cv2.imread(x)
    detect(img)
    pass
